...
### 입력 방식
🎤·🖼️ 버튼이 채팅 입력창 **내부 왼쪽**에 내장되어 있습니다.
...
| 버튼 | 기능 |
|------|------|
| 🎤 마이크 | 클릭 후 말하면 Whisper AI가 자동 전사·전송 (한국어 포함 100개 언어) |
| 🖼️ 이미지 | 클릭하면 파일 선택창이 열림, JPG·PNG·WEBP·GIF 첨부 후 전송 |
| ⌨️ 텍스트 | 입력창에 직접 타이핑 후 Enter 또는 ↑ 버튼으로 전송 |

- **중복 방지** — MD5 해시로 동일 음성 녹음 재처리 차단
- **Vision 분석** — 이미지 첨부 시 GPT-4o Vision으로 간판 번역·음식 식별·장소 인식 등
- **이미지 미리보기** — 전송 전 썸네일 확인 및 ✕ 버튼으로 제거 가능
- **자동 초기화** — 이미지 전송 후 업로더 자동 리셋
